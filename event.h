...
class CivilizationDeath : public Event {
 public:
  CivilizationDeath(double timestamp, EventHandler* handler, int civilization_id)
...
class EventHandler {
 public:
  virtual std::vector<std::unique_ptr<Event>> Handle(
      const std::unique_ptr<Event>& event, Universe* universe) = 0;
};

class CivilizationBirthHandler : public EventHandler {
 public:
  CivilizationBirthHandler(double birth_interval,
      std::default_random_engine* generator, CivilizationFactory* factory,
  ...
  std::vector<std::unique_ptr<Event>> Handle(
